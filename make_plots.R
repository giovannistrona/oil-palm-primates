...
b<-barplot(rbind(a$km2_low,a$km2_intermediate,a$km2_high),col=rbind(as.character(a$color),as.character(a$color),as.character(a$color)),beside=T,names=a$category, xlab='oil palm suitability - primate vulnerability',ylab=expression(paste("extent ","(","km"^"2"," ×"," 10"^"4",")")),cex.axis=1.5,cex.lab=1.5,cex=1.5)
...
res_acc_2<-aggregate(res_acc[,3]~res_acc[,1],FUN='mean')
res_carbon_2<-aggregate(res_carbon[,3]~res_carbon[,1],FUN='mean')
res_rand_2<-aggregate(res_rand[,3]~res_rand[,1],FUN='mean')
res_suit_2<-aggregate(res_suit[,3]~res_suit[,1],FUN='mean')
res_best_2<-aggregate(res_best[,3]~res_best[,1],FUN='mean')


res_acc_min2<-aggregate(res_acc[,3]~res_acc[,1],FUN='min')
res_carbon_min2<-aggregate(res_carbon[,3]~res_carbon[,1],FUN='min')
res_rand_min2<-aggregate(res_rand[,3]~res_rand[,1],FUN='min')
res_suit_min2<-aggregate(res_suit[,3]~res_suit[,1],FUN='min')
res_best_min2<-aggregate(res_best[,3]~res_best[,1],FUN='min')


res_acc_max2<-aggregate(res_acc[,3]~res_acc[,1],FUN='max')
res_carbon_max2<-aggregate(res_carbon[,3]~res_carbon[,1],FUN='max')
res_rand_max2<-aggregate(res_rand[,3]~res_rand[,1],FUN='max')
res_suit_max2<-aggregate(res_suit[,3]~res_suit[,1],FUN='max')
res_best_max2<-aggregate(res_best[,3]~res_best[,1],FUN='max')
...
par(mfrow=c(1,2))
par(mgp=c(2.00,0.75,0))
plot(res_acc_[,1],res_acc_[,2],type='l',col='white',lwd=2,xlab=expression(paste("oil palm expansion ","(","km"^"2"," ×"," 10"^"4",")")),ylab=expression(paste("average primate range loss ","(","km"^"2"," ×"," 10"^"4",")")) ,las=1,cex.axis=1.2,cex.lab=1.2,main='a')
...
lines(res_rand_[,1],res_rand_[,2],col='black',lwd=1,lty=2)

legend(0,14,c('suitability','accessibility','carbon','vulnerability','random'),col=c(cols[1],cols[2],cols[3],cols[4],'black'),lty=c(1,1,1,1,2),lwd=2,cex=0.8,bty = 'n')

plot(res_acc_2[,1],res_acc_2[,2],type='l',col='white',lwd=2,xlab=expression(paste("oil palm expansion ","(","km"^"2"," ×"," 10"^"4",")")),ylab='cumulative number of species\n losing more than 30% of range',las=1,cex.axis=1.2,cex.lab=1.2,main='b')
...
lines(res_rand_2[,1],res_rand_2[,2],col='black',lwd=1,lty=2)


legend(0,80,c('suitability','accessibility','carbon','vulnerability','random'),col=c(cols[1],cols[2],cols[3],cols[4],'black'),lty=c(1,1,1,1,2),lwd=2,cex=0.8,bty = 'n')
...
res_vuln_2<-aggregate(res_vuln[,3]~res_vuln[,1],FUN='mean')
res_vuln_min2<-aggregate(res_vuln[,3]~res_vuln[,1],FUN='min')
res_vuln_max2<-aggregate(res_vuln[,3]~res_vuln[,1],FUN='max')
...
pdf('all_scenarios.pdf',width=12,height=16)
par(mfrow=c(4,3))
for (mod in models){
	...
	plot(res_mod_[,1],res_mod_[,2],type='l',col='white',lwd=2,xlab="",ylab="",las=1,cex.axis=1.8,cex.lab=1.8,cex.main=2,main=gsub('_',' + ',mod))
...
#legend(0,14,c(mod,'random','vulnerability'),col=c('steelblue','black','red'),lty=c(1,2,1),lwd=2,cex=0.8,bty = 'n')
...
par(mfrow=c(4,3))
for (mod in models){
	res_mod<-read.csv(paste0('res_',mod,'.csv'),header=F)
	res_mod[,1]<-res_mod[,1]/10000
	res_mod[,2]<-res_mod[,2]/10000

	res_mod_2<-aggregate(res_mod[,3]~res_mod[,1],FUN='mean')
	res_mod_min2<-aggregate(res_mod[,3]~res_mod[,1],FUN='min')
	res_mod_max2<-aggregate(res_mod[,3]~res_mod[,1],FUN='max')

	par(mgp=c(2.00,0.75,0))
#plot(res_mod_2[,1],res_mod_2[,2],type='l',col='white',lwd=2,xlab=expression(paste("oil palm expansion ","(","km"^"2"," ×"," 10"^"4",")")),ylab='cumulative number of species\n losing more than 30% of range',las=1,cex.axis=1.8,cex.lab=1.8,cex.main=2,main=gsub('_',' + ',mod))
plot(res_mod_2[,1],res_mod_2[,2],type='l',col='white',lwd=2,xlab='',ylab='',las=1,cex.axis=1.8,cex.lab=1.8,cex.main=2,main=gsub('_',' + ',mod))
...
#legend(0,80,c(mod,'random','vulnerability'),col=c('steelblue','black','red'),lty=c(1,2,1),lwd=2,cex=0.8,bty = 'n')
...
res_suit_2<-aggregate(res_suit[,3]~res_suit[,1],FUN='mean')
res_best_2<-aggregate(res_best[,3]~res_best[,1],FUN='mean')
res_opt_2<-aggregate(res_opt[,3]~res_opt[,1],FUN='mean')


res_suit_min2<-aggregate(res_suit[,3]~res_suit[,1],FUN='min')
res_best_min2<-aggregate(res_best[,3]~res_best[,1],FUN='min')
res_opt_min2<-aggregate(res_opt[,3]~res_opt[,1],FUN='min')


res_suit_max2<-aggregate(res_suit[,3]~res_suit[,1],FUN='max')
res_best_max2<-aggregate(res_best[,3]~res_best[,1],FUN='max')
res_opt_max2<-aggregate(res_opt[,3]~res_opt[,1],FUN='max')
...
par(mfrow=c(1,2))
par(mgp=c(2.00,0.75,0))
plot(res_suit_[,1],res_suit_[,2],type='l',col='white',lwd=2,xlab=expression(paste("oil palm expansion ","(","km"^"2"," ×"," 10"^"4",")")),ylab=expression(paste("average primate range loss ","(","km"^"2"," ×"," 10"^"4",")")) ,las=1,cex.axis=1.2,cex.lab=1.2,main='a')
...
legend(0,14,c('suitability','vulnerability','optimization'),col=c(cols[1],cols[4],cols[5]),lty=c(1,1,1),lwd=2,cex=0.8,bty = 'n')

plot(res_suit_2[,1],res_suit_2[,2],type='l',col='white',lwd=2,xlab=expression(paste("oil palm expansion ","(","km"^"2"," ×"," 10"^"4",")")),ylab='cumulative number of species\n losing more than 30% of range',las=1,cex.axis=1.2,cex.lab=1.2,main='b')
...
legend(0,80,c('suitability','vulnerability','optimization'),col=c(cols[1],cols[4],cols[5]),lty=c(1,1,1),lwd=2,cex=0.8,bty = 'n')
